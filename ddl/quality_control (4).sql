...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 10, 2016 at 07:28 AM
...
--
-- Stand-in structure for view `item_inspection`
--
...
--
-- Structure for view `item_inspection`
--
DROP TABLE IF EXISTS `item_inspection`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `item_inspection`  AS  select `itm`.`itm_serie` AS `itm_serie`,`anw`.`anw_id` AS `anw_id`,`anw`.`anw_value` AS `anw_value`,`ins`.`ins_id` AS `ins_id`,`ins`.`ins_date` AS `ins_date` from ((`inspection` `ins` join `answers` `anw` on((`ins`.`anw_id` = `anw`.`anw_id`))) join `items` `itm` on((`anw`.`itm_serie` = `itm`.`itm_serie`))) ;
